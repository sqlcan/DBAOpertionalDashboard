<#
.SYNOPSIS
Update-SQLOpDatabaseRole

.DESCRIPTION 
Update-SQLOpDatabaseRole

.PARAMETER ServerInstance
SQL Server instance name for which the database roles are being updated.

.PARAMETER Data
Role data from Get-SIDatabaseRole.
...
.EXAMPLE
Update-SQLOpDatabaseRole -ServerInstance Contoso -Data $Data
...
#>
function Update-SQLOpDatabaseRole
{
    [CmdletBinding()] 
    param( 
    ...
    try
    {
        ...
        $TSQL = "EXEC Staging.TableUpdates @TableName=N'DatabaseRole', @ModuleVersion=N'$ModuleVersion'"	
        ...
        $TSQL = "MERGE Security.DatabasePrincipal AS Target
                USING (SELECT RoleName, RoleType FROM Staging.DatabaseRole WHERE ProcessID = $ProcessID) AS Source (RoleName, RoleType)
		...
		$TSQL = "DELETE FROM Staging.DatabaseRole WHERE ProcessID = $ProcessID"
		Write-StatusUpdate -Message $TSQL -IsTSQL
        Invoke-Sqlcmd -ServerInstance $Global:SQLOpsDBConnections.Connections.SQLOpsDBServer.SQLInstance `
                    -Database $Global:SQLOpsDBConnections.Connections.SQLOpsDBServer.Database `
                    -Query $TSQL
